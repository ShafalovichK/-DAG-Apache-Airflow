...
Original file is located at
    https://colab.research.google.com/drive/178Zgo8-dPhiojv1u6YxsSeClodkGnHDl
"""
...
DAG_ID = 'Shafalovich_Kseniya_dag1'  # имя DAG
default_args = {
...
def DownloadData(date, **kwargs):
    ...
    data_dir = 'C:\\Users\\Asus\\airflow-docker\\dags'  # временная директория для сохранения данных
...
def ProcessProduct(date, **kwargs):
    ...
    :param date: дата в формате 'YYYY-MM-DD'
    """
    data_path = f'C:\\Users\\Asus\\airflow-docker\\dags\\profit_table.csv'  # путь к файлу данных
    ...
    ti = kwargs['ti']
    ti.xcom_push(key='transformed_data', value=final_df.to_csv(index=False))  # Сохраняем трансформированные данные в XCom

# Шаг 3: Функция для загрузки данных
def LoadData(date, **kwargs):
    """
    Функция для загрузки данных в csv-файл.
    :param date: дата в формате 'YYYY-MM-DD'
    """
    ti = kwargs['ti']
    csv_data = ti.xcom_pull(key='transformed_data')
    df = pd.read_csv(pd.compat.StringIO(csv_data))

    product_list = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j']  # список продуктов
    for product in product_list:
        base_dir = 'C:\\Users\\Asus\\airflow-docker\\dags'  # временная директория для сохранения файла
...
with DAG(
    ...
    transformation_task = PythonOperator(
        task_id='transformation',  # уникальный идентификатор задачи для трансформации данных
    ...
    load_task = PythonOperator(
        task_id='load',  # уникальный идентификатор задачи для загрузки данных
        python_callable=LoadData,  # функция, которая будет вызвана для выполнения задачи загрузки данных
        op_kwargs={'date': '{{ ds }}'},  # передаем аргумент 'date' в функцию, который будет заменен на дату запуска DAG (Execution Date)
        provide_context=True  # включаем контекст
    )

    extraction_task >> transformation_task >> load_task  # задаем порядок выполнения задач